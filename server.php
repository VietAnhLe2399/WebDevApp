<?php
    ...
    $cf_psw_sign_up = "";
    $error = array();
    $db = mysqli_connect("localhost", "root", "", "quanlycuusv");
    ...
    if(isset($_POST['register'])) {
        ...
        if(($psw_sign_up == $cf_psw_sign_up) && (preg_match('/^[a-zA-Z0-9]{5,}$/', $urn_sign_up))) {
            $sql = "INSERT INTO user(username, password_user, role_user, idcuusinhvien) VALUES ('$urn_sign_up', '$psw_sign_up', 'normal', '17020000')";
            mysqli_query($db, $sql);
            $sql2 = "INSERT INTO cuusinhvien(idcuusinhvien, anh, idnamhoc, gioitinh, gioithieu) VALUES ('17020000', 'VNU-UET.jpg', '2017', 'Nam', 'Sinh viên UET - VNU')";
            mysqli_query($db, $sql2);
			session_unset();
            $_SESSION['username'] = $urn_sign_up;
            $_SESSION['check_user'] = 1;
			echo "<script> alert('Đăng ký thành công, mời đăng nhập lại');</script>";
        ...
        if(mysqli_num_rows($result) == 1) {
            ...
            if($arr_info_signin['idcuusinhvien'] == NULL) {
                ...
                if($arr_info_user['gioithieu'] == NULL) {
                    $_SESSION['gioithieu'] = "Sinh viên UET - VNU";
                }
                else {
